...
class TinyTokenizer(object):
	...
	def __init__(self):
		pattern =  '''
						(?:https?://[^\s]+)      # don't split URLs 
						|
						(?:\[SMILEY:[^\]]+\])    # Something something
						|
						(?:\[img\ssrc='[^']*'\])  # Something something
						|
						(?:\[object\ssrc='[^']*'\]) # Something something
						|
						(?:\[/?[biu]\])          # Something something
						|
						(?:HABABAM_USER_\S+)     # Exceptions
						|
						(?:[+\-]?\d+[,/.:-]\d+)  # Numbers, including fractions, decimals.
						|
						(?:[\w]+[-]+[\w]+)       # don't split words with dashes
						|
						(?:[\w]+[']+[\w]+)       # don't split words with single quotes, they're Turkish
						|
						(?:[\w_]+)               # Words without dashes
						|
						(?:\S)                   # Everything else that isn't whitespace.
						'''
		self.word_re = regex.compile(pattern, regex.VERBOSE | regex.UNICODE | regex.I)         # use regex for safe unicode handling
